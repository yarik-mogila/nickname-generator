...
import io.github.yarikmogila.nickgen.common.DictionaryNicknameGenerator;
...
final class NicknameGenerationFacade {
    ...
    NicknameGenerationFacade() {
        this(new DictionaryNicknameGenerator());
    }
    ...
    List<NicknameResult> generate(int count, NicknameLocale locale, NicknameTemplate template, String seedText) {
        Long seed = parseSeed(seedText);
        return generator.generate(new GenerationRequest(count, locale, template, seed));
    }
